...
///
/// On macOS, `apps_json` should contain app signing or bundle identifiers such
/// as `["com.roblox.RobloxPlayer"]`. Common Roblox Windows aliases are
...
///
/// JSON contract:
/// `{ \"region\": \"singapore\", \"apps\": [\"com.roblox.RobloxPlayer\"], \"custom_relay_server\": \"relay.example.com:51821\", \"auto_routing\": { \"enabled\": true, \"whitelisted_regions\": [\"US East\"] }, \"forced_servers\": {\"us-east\": \"us-east-nj\"} }`
///
/// `custom_relay_server` and `forced_servers` are optional additive fields.
/// Legacy payloads with only `region`/`apps` remain valid. On macOS, `apps`
/// should be signing or bundle identifiers; common Roblox Windows aliases are
...
///   "tunneled_processes": ["com.roblox.RobloxPlayer"],
///   "error": null
/// }
/// ```
#[no_mangle]
...
///
/// On macOS, the returned values are app signing identifiers as reported by the
...
///
/// This is a no-op on macOS because the transparent proxy provider reports
/// active flows directly.
/// Returns 0 on success, negative on error.
#[no_mangle]
...
///
/// On macOS, `process_name` is the intercepted app signing identifier.
#[no_mangle]
